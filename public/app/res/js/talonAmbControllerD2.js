(function(){
    ...
    angular
        .module('RegPortal')
        .controller('TalonAmbControllerD2', [
            '$cookies',
            'ApiFactory',
            'NsiFactory',
            'CheckModelFactory',
            'TalonAmbModelFactoryD2',
             TalonAmbControllerD2
        ]);

    function TalonAmbControllerD2($cookies, ApiFactory, NsiFactory, CheckModelFactory, TalonAmbModelFactory)
    {
        const vm = this;
        ...
        vm.checkEditorModel = CheckModelFactory.new(vm.lpu, 'D2');
        ...
        vm.getDSMetodHmp = getDSMetodHmp;
        ...
        function init() {
            ...
            vm.isDS_Onk();
            vm.getUsls();
            vm.getRefBooks(['V002', 'V020', 'V025', 'V027', 'V021', 'V022', 'V028', 'V029', 'V006', 'V008', 'V014', 'V009', 'V012', 'V010']);
        }
        ...
        function getDSVidHmp() {
            const label = vm.talonEditorModel.zl_list.zap.z_sl.sl[vm.slIndex].vid_hmp;
            return (label != undefined && label != null && label != 0 && label != '' && label && '0') ? label : 'Нет';
        }

        function getDSMetodHmp() {
            const label = vm.talonEditorModel.zl_list.zap.z_sl.sl[vm.slIndex].metod_hmp;
            return (label != undefined && label != null && label != 0 && label != '' && label && '0') ? label : 'Нет';
        }
        ...
        function getV018() {
            const hvidStr = vm.filters.refBooksDinamic.V018;
            vm.apiRecords.url(['refbooks', `search?name=V018&HVIDNAME=${hvidStr}`], (data) => {
                vm.refBooksDinamic.V018 = data;
            });
        }

        function getV019() {
            const metodHmpStr = vm.filters.refBooksDinamic.V019;
            vm.apiRecords.url(['refbooks', `search?name=V019&HMNAME=${metodHmpStr}`], (data) => {
                vm.refBooksDinamic.V019 = data.map((V019) => {
                    const _name = vm.refBooks.V022.filter((V022) => {
                        return V022.IDMPAC == V019.IDMODP;
                    })

                    const name = (_name) ? _name.MPACNAME : 'Нет';
                    console.log(_name.MPACNAME);
                    V019.MPACNAME = name;
                    return V019;
                });
            });
       ...
       function getSpecializationName()
       {
            if (vm.talonEditorModel.zl_list.zap.z_sl.sl[vm.slIndex].prvs        && 
                ...
                const spec = vm.nsi.V021.list.filter((spec) => {
       ...
       function setPacientId()
       {
        ...
        vm.onkoDiagEditor = {
            ...
            push: function() {
                if (vm.onkoEventEditorModel._id != undefined) {  
                    const index = parseInt(vm.onkoEventEditorModel._id);
                    vm.onkoDiagEditorModel._id = vm.talonEditorModel.zl_list.zap.z_sl.sl[vm.slIndex].onk_sl[index].b_diag.length;
        ...
        vm.slkoefEditor = {
            ...
            push: function() {
                if (vm.slkoefEditorModel._id == undefined) {
                    const length = vm.talonEditorModel.zl_list.zap.z_sl.sl[vm.slIndex].ksg_kpg.sl_koef.length;
